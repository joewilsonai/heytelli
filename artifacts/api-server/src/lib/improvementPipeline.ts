...
export function buildGithubIssueDraft(input: {
  ...
  const body = [
    ...
    ``,
    `## Repo Visibility`,
    `This issue lives in a private repo, but it is still GitHub-visible to repo tools, CI, and integrations. Keep it sanitized and do not copy private database rows into the issue, PR, prompt, or logs.`,
    ``,
    `## Privacy Note`,
